classdef Violin < handle
    ...
    properties
        ...
        ScatterPlotOut % scatter plot of outlier data points
...
%             obj.SepOutlierPoints = args.SepOutlierPoints;
            hold('on');
...
%             violin_lim_quantiles = [0,1]; % min, max
            violin_lim_quantiles = [0.01, 0.99];
            data_lims = quantile(data,violin_lim_quantiles);            

            [density, value] = ksdensity(data, 'bandwidth', args.Bandwidth);
            density = density(value >= data_lims(1) & value <= data_lims(2));
            value = value(value >= data_lims(1) & value <= data_lims(2));
%             value(1) = min(data);
%             value(end) = max(data);
            value(1) = data_lims(1);
            value(end) = data_lims(2);
...
%                 jitterstrength = interp1(value, density*width, data);
                jitterstrength = max(density)*width; 
            else % all data is identical:
                jitterstrength = density*width;
            end
            rng(1); % seed for repeatable plots
            jitter = 2*(rand(size(data))-0.5);
            if args.SepOutlierPoints
                obj.ScatterPlot = ...
                    scatter(pos + jitter(data>lowhisker & data<hiwhisker).*jitterstrength, ...
                            data(data>lowhisker & data<hiwhisker), 'filled','SizeData',2);
                obj.ScatterPlotOut = ...
                    scatter(pos + jitter(data<lowhisker | data>hiwhisker).*jitterstrength,...
                            data(data<lowhisker | data>hiwhisker), 'filled','SizeData',2);
            else
                obj.ScatterPlot = ...
                scatter(pos + jitter.*jitterstrength, data, 'filled','SizeData',2);                
            end
            % plot the violin
...
%             obj.ScatterPlot.MarkerFaceColor = color;
            obj.ScatterPlot.MarkerFaceColor = [0.6 0.6 0.6];
            obj.ScatterPlotOut.MarkerFaceColor = 'k';
        ...
        function set.ViolinAlpha(obj, alpha)
            ...
            obj.ScatterPlotOut.MarkerFaceAlpha = alpha;
        ...
        function set.ShowData(obj, yesno)
            if yesno
                ...
                obj.ScatterPlotOut.Visible = 'on';
                uistack(obj.ScatterPlot,'bottom');
            else
                obj.ScatterPlot.Visible = 'off';
                obj.ScatterPlotOut.Visible = 'off';
            end
        end
        ...
        function yesno = get.ShowData(obj)
            ...
            if ~isempty(obj.ScatterPlotOut)
                yesno = strcmp(obj.ScatterPlotOut.Visible, 'on');                
            end
        end
        ...
        function set.SepOutlierPoints(obj, yesno)
            if ~isempty(obj.SepOutlierPoints)
                if yesno
                    obj.ScatterPlotOut.Visible = 'on';
                else
                    obj.ScatterPlotOut.Visible = 'off';
        ...
        function yesno = get.SepOutlierPoints(obj)
            if ~isempty(obj.SepOutlierPoints)
                yesno = strcmp(obj.ScatterPlotOut.Visible, 'on');
            end
        end
