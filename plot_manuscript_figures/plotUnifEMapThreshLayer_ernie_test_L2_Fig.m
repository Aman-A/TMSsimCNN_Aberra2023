%% Plot unifE map threshold on l2/3 surface
function plotUnifEMapThreshLayer_ernie_test_L2_Fig(save_fig,cbar_mode,save_data)
if nargin == 0
   save_fig = 0; 
...
if opts.plot_vertices == 1
    opts.err_cax_lims = [-23 67]; % -23 67
    opts.fig_fold = 'figs';
else
    if strcmp(opts.plot_errs,'per_err') % median percent error
        opts.err_cax_lims = [-31.87838 97.113]; % plot_vertices = 0
        opts.fig_fold = 'figs2'; % figs - plot_vertices 1, figs2 - plot_vertices 2, figs3 - plot_vertices 2 and per_err_med
    elseif strcmp(opts.plot_errs,'per_err_med') % percent error of median threshold
        opts.err_cax_lims = [-32.49 98.22];
        opts.fig_fold = 'figs3'; % figs3 - plot_vertices 2 and per_err_med
    end    
end
opts.z_lims = []; 
opts.save_fig = save_fig; 
opts.fig_name_suffix = 'L2'; 
[threshEs,threshEsM,plot_errs] = plotUnifEMapThreshLayer(model_prefix_pre_base,...
                                            layer_set_num,nrn_pop_names,...
                                            Efield_name,opts);
...
elseif cbar_mode == 2 && save_fig
    plot_vert_cbars_figs(layer_num,opts.cax_lims,opts.err_cax_lims,...
                        opts.save_fig,2,opts.fig_fold); 
end                    
end                    